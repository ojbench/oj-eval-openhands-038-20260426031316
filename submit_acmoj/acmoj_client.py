...
def main():
    ...
    # Submit code
    submit_parser = subparsers.add_parser("submit", help="Submit code. If --language/--code-file omitted, submit current Git repo URL.")
    ...
    submit_parser.add_argument("--language", type=str, required=False,
                               help="Programming language (e.g., cpp, c). If omitted, will submit Git repo URL.")
    submit_parser.add_argument("--code-file", type=str, required=False,
                               help="Path to the source code file. Required if --language is provided.")
    ...
    if args.command == "submit":
        if args.language and args.code_file:
            ...
            # Submit current Git repository URL
            try:
                url = subprocess.check_output(["git", "remote", "get-url", "origin"], text=True).strip()
            except subprocess.CalledProcessError:
                print("Error: Failed to get Git remote URL. Ensure this is a git repo with 'origin' remote.")
                exit(1)
